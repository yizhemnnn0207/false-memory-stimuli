...
var conversation_history = [{
	"role": "system",
	"content": "作為一個AI警察，您的職責是提供有關犯罪現場的訊息。犯罪現場涉及一起發生在便利店的搶劫案。實際犯罪發生在報警之前的某個時間。您將遵循[系統記憶]提供的指示。"
}]; // 創建一個全局變量來存儲對話歷史
...
function add_user_bubble(input_message) {
	...
	conversation_history.push({ role: "user", content: make_prompt(input_message) });
...
function call_Gemini(following_phrase) {
	...
	const url = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=AIzaSyBcSs5Avdwt9DklyA9xo35uKnh3to9CdzU";
...
let ragPrompt = "[系統記憶] 用40-60個字，首先，評估用戶的回答是否與準確的事實相符；接著，清楚指出正確的事實；最後，將括號中的正確資訊自然融入敘述中（不要逐字重複括號內容，也不要在句末再重覆一次）。";
...
function make_prompt(user_input) {
	// 第1-5題維持原本模式，第6題之後使用 CoT 格式
	// total_dialogue: 3=第1題, 4=第2題, 5=第3題, 6=第4題, 7=第5題, 8=過渡, 9=第6題開始
	if (total_dialogue >= 3 && total_dialogue <= 7) {
		// 第1-5題：使用簡單正向回饋，不包含任何CoT推理或開頭語句
		let varPrompt = addSentencesWithVaryingProbabilities(positivePrompt, positiveSentenceStructures);
		return varPrompt + "\n\n" + user_input;
	} else if (total_dialogue >= 9) {
		// 第6題之後：使用 CoT 推理格式，以「我們一起來逐步推理整個案發過程」開頭
		const cot_prompt = `請您以推理討論的方式回答用戶的問題。回答時請遵循以下格式：

開頭：「我們一起來逐步推理整個案發過程」這樣的自然語句，營造出您在和用戶共同檢視案件的感覺。
中間：針對用戶的回答進行討論
