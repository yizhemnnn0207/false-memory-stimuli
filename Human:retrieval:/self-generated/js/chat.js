...
function call_Gemini(following_phrase) {
	...
	const url = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=AIzaSyBzOTcAtOqGg6WjyjLgmkM310kSLCggLJk";
...
// 定義基本提示詞
let basePrompt = "[系統記憶] 請您以以下格式回答用戶的問題，並且每次都要附上兩個資料來源（可以是案件記錄、報告、學術論文等）：開頭：直接說明正確與否；中間：針對用戶的回答進行討論，並引用兩個資料來源（例如：[案件記錄A, 2025]、[警局報告B, 2025]、[目擊者紀錄C, 2025]）。請務必每次都以這個架構回答，並且資料來源要以[來源名稱, 年份]格式出現。重要：請仔細評估用戶的回答是否與事實相符。如果用戶的回答與事實相矛盾，請明確指出這一點並使用否定性的表達。";

// 已移除 RAG 引用格式（Q6–Q20 仍使用 basePrompt）
