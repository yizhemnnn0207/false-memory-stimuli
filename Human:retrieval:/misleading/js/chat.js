...
var conversation_history = [{
	"role": "system",
	"content": "作為一名，您的職責是提供有關犯罪現場的訊息。犯罪現場涉及一起發生在便利店的搶劫案。報警時間大約是週五晚上9點。實際犯罪發生在報警之前的某個時間。您將遵循[系統記憶]提供的指示。"
}]; // 創建一個全局變量來存儲對話歷史
...
function call_Gemini(following_phrase) {
	...
	const url = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=AIzaSyA8pyyQQStKPWkCRBqEZoWgWJuQ6j_MD64";
...
請您以以下格式回答用戶的問題，並且每次都要附上一個資料來源（可以是案件記錄、報告、學術論文等）：

開頭：直接說明正確與否
中間：針對用戶的回答進行討論，並引用資料來源（例如：[案件記錄A, ${citationYear}]、[警局報告B, ${citationYear}]、[目擊者紀錄C, ${citationYear}]）
...
[系統記憶] 用40-60個字，首先，開頭「根據案發現場的記錄，……」，明確指出用戶的回答與事實不符，並提供正確答案。引用資料來源（例如：[案件記錄A, ${citationYear}]、[警局報告B, ${citationYear}]、[目擊者紀錄C, ${citationYear}]）來支持正確答案。請務必每次都以這個架構回答，並且資料來源要以[來源名稱, 年份]格式出現。重要：所有引用的年份一律使用 ${citationYear}，不得使用其他年份。
`;
...
function analyzeUserResponse(userResponse, questionType) {
	...
	// 非Critical題目：檢查是否回答錯誤，如果錯誤則使用correction模式
	if (!isUserAnswerCorrect(userResponse, questionType)) {
		return "correction";
	}
	
	// 如果回答正確，使用experiment模式
	return "experiment";
}
...
function make_prompt(user_input, mode = "experiment", questionType = null) {
	...
	if (mode === "base") {
		...
		let varPrompt = addSentencesWithVaryingProbabilities(correction_prompt, sentenceStructures);
		return varPrompt + "\n\n" + user_input;
	} else {
